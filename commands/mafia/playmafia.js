...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption((option) =>
      option
        .setName("extra_mafia")
        .setDescription("Add +1 extra Mafia")
        .setRequired(false),
    )
    .addBooleanOption((option) =>
      option
        .setName("extra_jester")
        .setDescription("Add +1 extra Jester")
        .setRequired(false),
    )
    .addBooleanOption((option) =>
      option
...
async function runMafiaLogic(interaction, mode, forceStart) {
  ...
  const extraMafia = interaction.options.getBoolean("extra_mafia") || false;
  const extraJester = interaction.options.getBoolean("extra_jester") || false;
  ...
  if (mode === "simple" || mode === "rl") {
    ...
    if (extraMafia) mafiaCount += 1;
    if (extraJester) jesterCount += 1;
    ...
    try {
      // Send DM to all Mafia players (they see who the other Mafias are)
      const mafiaText =
      ...
      for (const j of jester) {
        await j.send(
          "You are **Jester**.\n\n" +
            "Your goal: Make people think you're the Mafia so they vote for you at the end.\n" +
      ...
      const simpleEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `**Mafia:** ${mafiaCount} player(s) have been chosen and DMed.\n` +
            `${jesterCount > 0 ? `**Jester:** ${jesterCount} player(s) have been chosen and DMed.\n` : ""}\n` +
            `**How to play:**\n` +
            `• The Mafia tries to throw/lose the game without being obvious.\n` +
            `• At the end of the round, everyone votes who they think the Mafia is.\n\n` +
            `**Scoring:**\n` +
      ...
      await reply("Could not DM the chosen role(s) — DMs probably closed.");
    }
  ...
  // ────────────────────────────────────────────────
  if (mode === "sabotage") {
    let mafiaCount = Math.max(1, Math.floor(totalPlayers * 0.2));
    let jesterCount = Math.floor(totalPlayers * 0.1);
    ...
    const publicEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `Voice channel: **${voiceChannel.name}**\nPlayers: ${totalPlayers}\n` +
          `Mafia: **${mafiaCount}**${extraMafia ? " (1 extra added)" : ""} | Jester: **${jesterCount}**${extraJester ? " (1 extra added)" : ""}`,
      )
      .addFields(
        {
    ...
    await reply("Sabotage roles assigned privately.\nGood luck! 🕵️");
    return;
  }
  ...
  if (mode === "kara") {
      ...
      .addFields(
        {
          name: "Setup",
          value:
            `• **${mafiaCount} Mafia**${extraMafia ? " (1 extra added)" : ""} — try to die 3 or more times during the raid (excluding wipes)\n` +
            `• **${jesterCount} Jester**${extraJester ? " (1 extra added)" : ""} — try to get voted as Mafia\n` +
    ...
    // Mafia DM - Includes all other Mafias
    const mafiaText =
      `**${mafiaNames}** are **Mafia**.\n\n` +
      `Your goal: **Die 3 or more times** during the raid (excluding wipes).\n` +
      `Do NOT make it obvious that you're trying to die!`;
